...
class TestReturnBothSentenceCleaning:
    ...
    def test_fields_sanitized_lists_only_changed_fields(self):
        d = PromptDefense()
        d.warmup_tier2()
        payload = {
            "summary": self._INJECTION,
            "benign_note": "Please review the attached document at your convenience.",
        }
        r = d.defend_tool_result(payload, "hris_get")
        assert "summary" in r.fields_sanitized
        assert "benign_note" not in r.fields_sanitized
        # sanitize_content=False -> nothing cleaned -> empty.
        d2 = PromptDefense(sanitize_content=False)
        d2.warmup_tier2()
        r2 = d2.defend_tool_result(payload, "hris_get")
        assert r2.fields_sanitized == []
